...
`page`/`pageSize` — instead of adding those fields to `TState`, `src/accidents/pagination-state.ts`
...
extra. `selectVisiblePage` in `src/accidents/pagination-state.ts` proves this: it hands the
...
`paginate<T>(items, page, pageSize)` in `src/accidents/pagination.ts` doesn't import `TState` or
...
Check any file in `src/essence`: zero imports from `src/essence-view` or `src/accidents`. The
dependency arrow only ever points one way — accidents import essence, never the reverse. This
