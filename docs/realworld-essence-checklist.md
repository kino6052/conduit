...
- [x] **Login** (`/login`) — establishing "who you are" (→ `LoginPage`, `src/accidents/view/react/pages.ts`, reachable via the header's Login tab and `#/login`)
  ...
  - [x] **Two real pages, not one collapsed form** — Sign In and Sign Up are now separate pages (`#/login`, `#/register`), each with its own heading, submit-button label, and a cross-link between them, matching the real spec's page shape (`docs/spec/pages.md`'s Sign in/Sign up entries). What still doesn't (and can't, without a much larger essence change): both pages submit through the exact same underlying action, `TSignIn.signIn(name, password)` — this app's name-only identity model has no account record to tell "already exists" from "brand new" apart, so there's genuinely nothing for the two forms to differ on beyond page/wording (→ `LoginPage`/`RegisterPage`, `pages.ts`; `SignIn`'s `submitLabel`, `components.ts`)
...
9. ~~Sign In/Sign Up collapsed into one name+password form~~ — partially resolved: they're two real pages now (`#/login`/`#/register`), matching the spec's page shape. Still open, and likely permanent short of a much larger essence change: the real spec's two forms have genuinely different *fields* (Sign In: email+password; Sign Up: username+email+password), which this app's passwordless, account-free identity model has no way to distinguish -- both pages submit through the same `signIn(name, password)` action. Confirmed as a deliberate simplification for this exercise, not a gap left to close.
