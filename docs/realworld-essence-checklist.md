...
- [ ] **A dedicated settings page — not yet built; the earlier "decided against" call was wrong.** Checked against the real spec (`realworld-docs.netlify.app/specifications/frontend/settings-page`): the real Settings page edits exactly the fields the real User object carries — `image` (avatar URL), `username`, `bio`, `email`, `password` — all five real and spec-required, not manufactured. This checklist previously argued bio/avatar/email had "no perceivable correlate anywhere in this app's actual rendered output," but that was reasoning from this codebase's current gaps rather than the spec it's meant to be grounded in — see the bio/avatar correction above. Once bio/avatar become real essence fields, editing them needs a real page; only the name-changing-via-Login shortcut was ever a genuine substitute, and only for the one field (name) already essence-grounded here. Email/password have no analog in this app's sign-in model at all (sign-in is by name only, no password) — open question below on whether to add them just to match the spec's form shape.
...
- [ ] **No tab/link to Settings — a real gap, not a settled absence.** The real header nav (`realworld-docs.netlify.app/specifications/frontend/header-navigation`) shows, once signed in: Home, New Article, Settings, and the user's own name *with avatar image* linking to their profile — four items, not the two ("New Article", "Sign Out (name)") this app currently shows. Once a Settings page exists (see above), this header needs a real link to it, and the signed-in name should link to your own Profile rather than just reading as a Sign Out control.
...
- [x] **Home** (`/`) — the article feed and how to narrow it down (→ `HomePage`, `src/accidents/view/react/pages.ts`)
  ...
  - [ ] **Missing the banner tagline** — the real Home page's banner reads "conduit" plus "A place to share your knowledge." underneath (`realworld-docs.netlify.app/specifications/frontend/home-page`); this app has no banner text at all, just the feed straight away. Cosmetic, but a real, checkable omission
...
- [x] **New article** / **Edit article** (`/editor`, `/editor/:article`) — writing (→ `EditorPage`, `src/accidents/view/react/pages.ts`, reachable via the header's New Article tab, once signed in, and `#/editor`/`#/editor/<title>`)
  ...
  - [ ] **Tag input shape diverges from spec** — the real Editor (`realworld-docs.netlify.app/specifications/frontend/editor-page`) enters tags one at a time (type, hit Enter, get a removable pill), building a list; this app takes one comma-separated text field parsed into `tags: string[]` on submit. Same resulting essence (a list of tag strings), different accident — flagging since the pill UI is also how tags get *removed* individually, which a single text field makes clumsier, not just cosmetically different
...
- [x] **Article** (`/article/:title`) — reading, interacting, one article (→ `ArticlePage`, `src/accidents/view/react/pages.ts`)
  ...
  - [ ] **Missing the author's avatar image** — the real Article banner (`realworld-docs.netlify.app/specifications/frontend/templates`) shows the author's profile picture, linked, next to their name and the publication date, both in the banner and on every comment card. This app shows the name alone, no image — same underlying gap as the Profile page correction above (no avatar field exists in essence yet)
...
- [x] **Profile** (`/profile/:authorName`) — an author, their articles (→ `ProfilePage`, `src/accidents/view/react/pages.ts`, reachable via any author name shown anywhere and `#/profile/<authorName>`)
  - [ ] **Author bio/avatar display — confirmed essence, not yet built.** Corrects the earlier "not yet decided" framing: the real spec's Profile page (`realworld-docs.netlify.app/specifications/frontend/templates`) always shows the profile image and bio text alongside the username; this page currently shows only the name
  - [ ] **Missing the "Edit Profile Settings" button** — shown on the real Profile page only when it's your own profile (in place of the Follow button, which only makes sense for someone else's); this app has no Settings page yet for it to link to (see the Settings correction above)
