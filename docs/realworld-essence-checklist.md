...
- [x] **Profile** (`/profile/:authorName`) — an author, their articles (→ `ProfilePage`, `src/accidents/view/react/pages.ts`, reachable via any author name shown anywhere and `#/profile/<authorName>`)
  - [ ] **Author bio/avatar display — confirmed essence, not yet built.** Corrects the earlier "not yet decided" framing: the real spec's Profile page (`docs/spec/pages.md`) always shows the profile image and bio text alongside the username; this page currently shows only the name
